...
def collate_fn_no_label(batch):
    waveforms = pad_sequence([torch.tensor(wave) for wave in batch], batch_first=True)
    return waveforms
...
class MyModel(torch.nn.Module):
    def __init__(self, pretrained_name: str):
        ...
        # layer re initiailzation
        self.model.projector.apply(self._init_weight_and_bias)
        self.model.classifier.apply(self._init_weight_and_bias)

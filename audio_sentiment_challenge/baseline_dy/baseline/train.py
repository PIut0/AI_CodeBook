...
from transformers import WhisperFeatureExtractor # 추가

warnings.filterwarnings(action='ignore')

train_serial = datetime.now().strftime("%Y%m%d_%H%M%S")
save_dir = f"/scratch/network/mk8574/audio_sentiment_challenge/baseline_dy/baseline/results/{train_serial}"
...
#feature_extractor = WhisperFeatureExtractor.from_pretrained(config.pretrained_name) #추가
...
train_dataset = MyDataSet(train_df, feature_extractor, mode='train', transforms=None, data_path=config.data_path)
valid_dataset = MyDataSet(valid_df, feature_extractor, mode='val', data_path=config.data_path)
...
optimizer = torch.optim.AdamW(
    [{"params": module.parameters(), "lr": config.lr if name in[ "classifier","fc"] else config.lr * 0.1} for name, module in model.named_children()],
    weight_decay=0.1,
)
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=3, verbose=True)
recorder = Recorder(save_dir)

best_model = train(model, creterion, train_loader, valid_loader, optimizer, scheduler, recorder, config, amp=config.amp, save_dir=config.logging*save_dir)

...
def train(model, creterion, train_loader, valid_loader, optimizer, scheduler, recorder=None, config=None, amp=False, save_dir=None):
    ...
    for epoch in range(1, config.max_epoch+1):
        train_loss = []
        model.train()
        for m in model.model.wav2vec2.feature_projection.parameters():
...
def train_noval(model, creterion, train_loader, optimizer, scheduler, recorder=None, config=None, amp=False, save_dir=None):
    device = config.device

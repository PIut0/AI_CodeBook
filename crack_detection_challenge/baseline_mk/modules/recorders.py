...
class Recorder():
    ...
    def __init__(self,
        ...
        self.weight_dir = os.path.join(record_dir, 'model')
    ...
    def save_weight(self, epoch: int, patience_counter, best_target)-> None:
        """Weight 저장
            amp 추가
        Args:
        ...
        weight_path = os.path.join(self.weight_dir, f'epoch{epoch}.pt')
        last_weight_path = os.path.join(self.weight_dir, 'last_model.pt')
        torch.save(check_point, weight_path)
        
        self.logger.debug(f"RECORDER | epoch {epoch}, checkpoint saved: {weight_path}")
    
    def save_by_condition(self, epoch: int, patience_counter, best_target) -> None:
        '''
            저장할 epoch의 조건 설정
    ...
    def save_plot(self, plots: list):
        ...
        for plot_name in plots:
            columns = []
            names = [f'val_{plot_name}',f'train_{plot_name}',f'{plot_name}']

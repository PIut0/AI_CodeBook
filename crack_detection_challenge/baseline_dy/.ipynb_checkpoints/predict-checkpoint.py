...
if __name__ == '__main__':
    ...
    # Set data directory
    test_dirs = os.path.join('/scratch/network/mk8574/crack_detection_challenge/data', 'test')
    test_img_paths = glob(os.path.join(test_dirs, 'images', '*.jpg'))
    ...
    #! Load weight
    check_point_path = os.path.join(prj_dir, 'results', 'train', config['train_serial'], 'model', f'epoch{pred_epoch}.pt')
    ...
    with torch.no_grad():
        ...
        for batch_id, (x, orig_size, filename) in enumerate(tqdm(test_dataloader)):
            filename = list(map(lambda x : x.replace('jpg', 'png'),filename))
    ...
    pred_files = glob(f'{pred_result_dir_mask}/*.png')
    with zipfile.ZipFile(f'/scratch/network/mk8574/crack_detection_challenge/baseline_youhan/results/submission{pred_serial}.zip', 'w') as zip:
